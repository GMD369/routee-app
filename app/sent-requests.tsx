...
export default function SentRequestsScreen() {
  const [sentRequests, setSentRequests] = useState<IncomingRequest[]>([]);
  ...
  async function loadSentRequests() {
    ...
    try {
      const data = await getSentRequests(); // fetch all statuses
      ...
      <SafeAreaView edges={["top"]} style={{ flex: 1 }}>
        ...
        <ScrollView contentContainerStyle={s.scrollContent} showsVerticalScrollIndicator={false}>
          {loadingRequests ? (
            ...
            <Text style={s.emptyStateText}>You haven't sent any requests.</Text>
          ) : (
