...
function defaultFormState(): VehicleFormState {
  return {
    make: "",
    model: "",
...
export default function AddVehicleScreen() {
  ...
  const [form, setForm] = useState<VehicleFormState>(() => defaultFormState());
  ...
  async function initialize() {
    ...
    try {
      ...
      setIsAllowed(driverProfile.verification_status === "verified");
    } catch (error) {
  ...
  async function onSubmit() {
    ...
    setSubmitting(true);
    try {
      const payload: VehicleCreateFormRequest = {
        make,
        model,
        year,
        color,
      ...
      Alert.alert(
        "Vehicle registered",
        `${formatVehicleTitle(created)} was added successfully.`,
        [
          {
            text: "View vehicle",
            onPress: () =>
  ...
  if (!isAllowed) {
    return (
      ...
      >
        <Text className="text-3xl font-black text-slate-900">Add Vehicle</Text>
  ...
  return (
    ...
    >
      <Text className="text-3xl font-black text-slate-900">Add Vehicle</Text>
      <Text className="mt-2 text-sm text-slate-500">
        Register a vehicle for your verified driver profile.
      </Text>
      ...
      <View className="mt-5 rounded-3xl border border-slate-200 bg-white p-4">
        ...
        >
          <Text className="text-sm font-semibold text-sky-600">
            {registrationDoc ? registrationDoc.name : "Select document"}
          </Text>
        </Pressable>
      </View>
